...
@section('content-header', 'Blog Categories')
    ...
    <div class="row">
            ...
            <div class="box box-danger">
                ...
                <div class="box-body table-responsive no-padding">
                    @if($categories->isEmpty())
                        <h3 style="text-align: center">There isn't any category!</h3>
                    @else
                        <table class="table table-hover">
                            ...
                            <tr>
                                ...
                                <th colspan="9">
                                    <form id="selections" action="{{ route('admin.blog.category.selections') }}" method="post">
                                        @csrf
                            ...
                            <tbody>
                                @foreach($categories as $category)
                                    <tr>
                                        <th>{{ $index++ }}</th>
                                        <td><input form="selections" type="checkbox" name="categories_id[{{ $category->id }}]"
                                                   class="categories"></td>
                                        <td>{{ $category->title }}</td>
                                        <td style="padding-top: 5px;">
                                            <a href="{{ route('admin.blog.category.edit', ['category' => $category->id]) }}"
                                                class="btn btn-success btn-sm" style="margin-top: 3px;">Edit</a>
                                            <form action="{{ route('admin.blog.category.destroy', ['category' => $category->id]) }}"
                        ...
                        <center style="margin-inline: 10px">
                            {{ $categories->onEachSide(1)->links('admin.layouts.pagination') }}
                        </center>
                    @endif
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h1 class="box-title">Create Category</h1>
                </div>
                <div class="box-body">
                    <div class="row">
...
@section('script')
    ...
    <script>
        ...
        document.getElementById('check_all').onclick = function () {
            for (let checkbox of document.getElementsByClassName('categories'))
